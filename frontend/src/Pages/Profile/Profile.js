...
import PollAnswer  from '../../MyComponents/PollAnswer/PollAnswer'
import './profile.css'
...
const Profile = () => {
    ...
    console.log("profile!!!",user.pollsVoted)
  ...
  <h6 className="m-b-20 m-t-40 p-b-5 b-b-default f-w-600">Polls Voted</h6>
                            ...
                            <h6 className="text-muted f-w-400">Not Polls Voted Yet !</h6>

                             :

                             allPolls.map((poll)=> user.pollsVoted.map(el=>el._id ===poll._id  ?
                             <div>                           
      ...
      {/* {allPolls.map(poll=>user.pollsVoted.map((el)=>el._id ===poll._id  ? <PollAnswer id={el._id} el={el} key={i}/>:null))} */}
